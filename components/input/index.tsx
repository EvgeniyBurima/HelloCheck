...
const getIcon = (icon: Icon): JSX.Element | string => {
  ...
  if (Component) {
    return (
      <span className="absolute left-0 top-0 items-center px-[10px] py-3">
        <Component size="18" />
      </span>
    );
  }
...
function Input({
  ...
  return (
    <label className={`relative w-full ${className || ''}`} htmlFor={name}>
      { icon && getIcon(icon)}
      <input
        className="placeholder:text-md placeholder:text-steel-200 block bg-white w-full border border-slate-300 rounded-md py-2 pl-9 pr-3"
